...
type OutsidePendingKey = OutsideControlKey | GarageKey | typeof GATE_KEY
...
function GarageDoorIcon({ open }: { open: boolean }) {
  ...
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className="outside-garage-icon">
      ...
      {/* black outline on top + sides only; bottom flush with building */}
      <path
...
/** Closed/open dual gate; uses currentColor to match garage button chrome. */
function GateIcon({ open }: { open: boolean }) {
  if (open) {
    return (
      <svg viewBox="0 0 24 24" aria-hidden="true" className="outside-garage-icon">
        <rect x="2" y="3.6" width="3.1" height="16.8" rx="0.45" fill="currentColor" />
        <rect x="18.9" y="3.6" width="3.1" height="16.8" rx="0.45" fill="currentColor" />
        {/* leaves edge-on */}
        <rect x="5.15" y="4.4" width="2.15" height="15.2" rx="0.25" fill="currentColor" />
        <rect x="16.7" y="4.4" width="2.15" height="15.2" rx="0.25" fill="currentColor" />
      </svg>
    )
  }
  ...
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className="outside-garage-icon">
      <rect x="2" y="3.6" width="3.1" height="16.8" rx="0.45" fill="currentColor" />
      <rect x="18.9" y="3.6" width="3.1" height="16.8" rx="0.45" fill="currentColor" />
      {/* left leaf with vertical bar cutouts */}
      <path
        fill="currentColor"
        fillRule="evenodd"
        d="M5.2 4.4h6.7v15.2H5.2V4.4zm1.05 1.15h.7v12.9h-.7V5.55zm1.45 0h.7v12.9h-.7V5.55zm1.45 0h.7v12.9h-.7V5.55zm1.45 0h.7v12.9h-.7V5.55zm1.45 0h.7v12.9h-.7V5.55z"
      />
      {/* right leaf */}
      <path
        fill="currentColor"
        fillRule="evenodd"
        d="M12.1 4.4h6.7v15.2H12.1V4.4zm1.05 1.15h.7v12.9h-.7V5.55zm1.45 0h.7v12.9h-.7V5.55zm1.45 0h.7v12.9h-.7V5.55zm1.45 0h.7v12.9h-.7V5.55zm1.45 0h.7v12.9h-.7V5.55z"
      />
    </svg>
  )
}
...
function GarageDoorControl({
  ...
  readOnly: boolean
  connected: boolean
  startPending: (key: OutsidePendingKey, desiredOn: boolean) => void
  clearPending: (key: OutsidePendingKey) => void
...
export function OutsideWidget() {
  ...
  const { pendingByKey, startPending, clearPending, reconcile } =
    usePendingToggles<OutsidePendingKey>({ giveUpMs: GATE_CLOSE_PENDING_MS })
  ...
  const actualByKey = useMemo(
    ...
    [controls, gate.isOpen, mainGarage.isOpen, workshopGarage.isOpen],
  )
